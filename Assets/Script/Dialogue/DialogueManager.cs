...
public class DialogueManager : MonoBehaviour
{
    ...
    /// <summary>当前对话的NPC是否刚接收了任务（离开NPC时触发显示信息面板）</summary>
    private bool taskJustAccepted = false;
    ...
    public void OnChoiceSelected(bool isTask)
    {
        ...
        if (isTask && !currentNPC.IsTaskCompleted)
        {
            // 进入任务线
    ...
    private void OnPhaseComplete()
    {
        switch (state)
        {
            ...
            case FlowState.TaskLine:
                // 任务线完毕 → 标记完成，直接结束对话
                currentNPC.IsTaskCompleted = true;
                taskJustAccepted = true;
                EndDialogue();
                break;
    ...
    private void ShowChoices()
    {
        ...
        bool showTask = !currentNPC.IsTaskCompleted;
    ...
    {
        // 如果刚接收了任务，重置玩家任务完成记录，显示任务信息面板
        if (taskJustAccepted)
        {
            taskJustAccepted = false;

            if (PlayerTaskManager.Instance != null)
            {
                PlayerTaskManager.Instance.AcceptTask();
            }
            else
            {
                Debug.LogWarning("[DialogueManager] 玩家身上没有 PlayerTaskManager，无法记录任务！");
            }
